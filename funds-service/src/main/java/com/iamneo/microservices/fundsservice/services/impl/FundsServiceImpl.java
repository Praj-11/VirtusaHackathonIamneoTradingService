...
//        amountAdd.setUserDetails(userDetailsFeignClient.getUserDetailsById(amountAdd.getUserId()).getBody());
    ...
    public FundsDto withdraw(Funds funds) {
        ...
        if (fundsRepository.findById(funds.getUserId()).isPresent()) {
            ...
            if(data.isPresent() && data.get().getAmount() > funds.getAmount())
    ...
    public Funds getFunds(String userId) {
        ...
        Optional<Funds> fundsResponse = fundsRepository.findById(userId);

        return fundsResponse.orElseGet(() -> new Funds(null, null));
    }
